...
- (id)initWithDelegate:(id)delegate {
    ...
    if (self) {
        ...
        [self setImage:[UIImage imageNamed:@"checkBox_select"] forState:UIControlStateSelected];
        [self addTarget:self action:@selector(checkboxBtnChecked) forControlEvents:UIControlEventTouchUpInside];
    }
    return self;
}

- (id)initWithNewDelegate:(id)delegate {
    self = [super init];
    if (self) {
        _delegate = delegate;
        ...
        [self setImage:[UIImage imageNamed:@"radio_select"] forState:UIControlStateSelected];
        [self addTarget:self action:@selector(checkboxBtnChecked) forControlEvents:UIControlEventTouchUpInside];
    }
    return self;
}
