...
+ (AFNetAPIClient *)sharedJsonClient {
    ...
    dispatch_once(&onceToken, ^{
        _sharedClient = [[AFNetAPIClient alloc] initWithBaseURL:[NSURL URLWithString:@"https://www.baidu.com/"]];
    });
...
{
    // /先导入证书
    NSString *cerPath = [[NSBundle mainBundle] pathForResource:@"lwork.hk" ofType:@"cer"]; //证书的路径
 ...
 * @return 返回字典
 */
-(NSDictionary *)parseJSONData:(NSString *)jsonString {
    if (jsonString == nil) {
        return nil;
    }
